...
bool restOneSand(vector<vector<bool>>& field, pair<int, int> sandPos) {
    do {
        ...
        if(nextPos.second == field[0].size() - 1) return false;
...
int calculateRestSendUnits(const vector<string> &lines)
{
    ...
    vector<vector<bool>> field(rightTop.first - leftBottom.first + 3, vector<bool>(leftBottom.second - rightTop.second + 1, true));
